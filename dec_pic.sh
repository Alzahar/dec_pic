...
-l       path for local copy / каталог с локальной копией из которой создаётся список строк;
-o       path for make list of strings / использовать для запуска данный каталог списка строк;

$0 -u \"http://example.com/one/two.html\" -l \"/path/to/local\" -o \"/path/git/proj\"
-u & -l & -o       скачать из \"-u\" в \"-l\", сформировать список строк в \"-o\" и запустить;

-u & -l        скачать в \"-l\" и сформировать список строк в нём-же;
-l & -o        из \"-l\" сформировать список строк в \"-o\" и запустить;
-u & -o        скачать в \"-l\" без формирования списка и запустить.
"
exit 0
fi
